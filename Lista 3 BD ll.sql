...
DELIMITER $$
CREATE PROCEDURE clientes_cadastrados ()
BEGIN
	select*from cliente;
END $$
DELIMITER ;

CALL clientes_cadastrados;

DELIMITER $$
CREATE PROCEDURE Produto_Barato (valor real)
BEGIN
	select*from produto
...
CALL Produto_Barato(3.00);

DELIMITER $$
CREATE PROCEDURE Valor_Total (codigo int)
BEGIN
	select precounitario*quantidade from produto as p
    join itemdopedido as ip on p.codigoproduto=ip.codigoproduto 
    where (p.codigoproduto=@codigo);
END $$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE Produto_cadastrado (v_codigo int, v_unidade char(20), v_descricaoproduto varchar(30),v_precounitario real)
BEGIN
IF((v_codigo!='')&&(v_unidade!='')&&(v_descricaoproduto!='')&&(v_precounitario!=''))THEN
INSERT INTO 
produto(codigoproduto,unidade,descricaoproduto,precounitario)VALUES(v_codigo,v_unidade,v_descricaoproduto,v_precounitario);
ELSE
SELECT 'Os valores inseridos sao INVALIDOS' AS Msg;
END IF;
END $$
DELIMITER ;

CALL Produto_cadastrado(5475,'mao','doce',1.50);
CALL Produto_cadastrado(2375,'mao','amarga',0.50);
CALL Produto_cadastrado(null,'','',null);

DELIMITER $$
CREATE PROCEDURE sp_Nome (nome varchar(30))
BEGIN
	select * from produto  
    where descricaoproduto LIKE nome ;
END $$
DELIMITER ;
