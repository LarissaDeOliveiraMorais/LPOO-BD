...
select e.nome, e.matricula from estudante as e join cursa as c 
on e.matricula=c.matricula_est join disciplina as d on d.codigo= c.cod_disc 
...
create trigger tg_funcionario_update after update on funcionario  for each row
insert log_funcionario (comando,Usuario,data_hora) 
values ('update',current_user(), current_date());

update funcionario set nome='joana darque' where cpf='0987654321';

select * from log_funcionario;

create trigger tg_funcionario_insert before insert on funcionario for each row 
insert into log_funcionario (comando,usuario,data_hora)
values ('INSERT',CURRENT_USER(),CURRENT_DATE());


insert into funcionario (cpf,nome,cargo) values ('112233489','Hitalo Servolo', 'Analista de Sistemas');
